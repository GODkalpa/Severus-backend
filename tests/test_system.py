...
async def check_websocket_connection():
    ...
    try:
        async with websockets.connect(WS_URL) as websocket:
            ...
            # Send a tiny silent audio chunk (one second of 16k mono 16-bit PCM silence = 32000 bytes)
            # Actually, just 1KB is enough to trigger the server's receive_bytes()
            print("Sending test audio chunk...")
            await websocket.send(bytes([0] * 1024))
            print("Waiting for response (Brain -> TTS loop)...")
            
            try:
                # We expect a binary response (audio)
...
async def main():
    ...
    if api_ok:
        ...
        if ws_ok:
            ...
            print("\n❌ WebSocket flow failed.")
    else:
        print("\n❌ API is unreachable. Start the server with 'python main.py' first.")
