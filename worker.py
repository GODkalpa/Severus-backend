...
def log_worker(message: str):
    """
    Helper to log background worker activity to both console and file.
    """
...
async def get_active_reminders():
    ...
    try:
        ...
        log_worker(f"❌ Error fetching reminders: {e}")
        return []
...
async def get_all_subscriptions():
    ...
    try:
        ...
        log_worker(f"❌ Error fetching subscriptions: {e}")
        return []
...
async def update_reminder_time(reminder_id: str):
    ...
    try:
        ...
        log_worker(f"✅ Reminder {reminder_id} timestamp updated.")
    except Exception as e:
        log_worker(f"❌ Error updating reminder timestamp: {e}")

async def run_context_engine():
    """
    The main 24/7 background loop for Severus.
    """
    log_worker("🧙‍♂️ [REVELIO] 24/7 Context Engine Initialized.")
    log_worker("Severus is now watching over your vitals and tasks.")
    ...
    while True:
        try:
            ...
            if not reminders:
                log_worker("Idle... No active reminders found.")
            elif not subscriptions:
                log_worker("Skipping... No device subscriptions registered.")
            else:
                ...
                for reminder in reminders:
                    ...
                    if is_one_off:
                        ...
                        if now >= due_at:
                            log_worker(f"🚨 ONE-OFF TIMER: {text}")
                            for sub in subscriptions:
                                sub_info = {
                                    "endpoint": sub["endpoint"], 
                                    "keys": {"p256dh": sub["p256dh"], "auth": sub["auth"]}
                                }
                                send_push_notification(sub_info, text, title="TIMED_ALARM")
                            ...
                            log_worker(f"⏳ Timer '{text}' check: Due in {mins} mins.")
                        continue
                    ...
                    if now >= next_notification:
                        log_worker(f"🚨 RECURRING ALERT: {text}")
                        for sub in subscriptions:
                            sub_info = {
                                "endpoint": sub["endpoint"], 
                                "keys": {"p256dh": sub["p256dh"], "auth": sub["auth"]}
                            }
                            send_push_notification(sub_info, text, title="SEVERUS_NUDGE")
                        await update_reminder_time(reminder_id)
                    else:
                        diff = next_notification - now
                        mins = int(diff.total_seconds() // 60)
                        log_worker(f"⏳ Recurring '{text}' check: Wait {mins} more mins.")

        except Exception as e:
            log_worker(f"❌ Context engine error: {e}")
